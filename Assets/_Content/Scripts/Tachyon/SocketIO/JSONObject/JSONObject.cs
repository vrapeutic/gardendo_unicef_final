...
public class JSONObject {
	...
	public enum Type { NULL, STRING, INTEGER, OBJECT, ARRAY, BOOL, BAKED, FLOAT }
...
//#endif
	...
	#endregion
    public bool IsFloat { get { return type == Type.FLOAT; } }
	public bool IsNumber { get { return type == Type.INTEGER; } }
		...
		if(type == Type.OBJECT) {
			...
			for(int i = 0; i < list.Count; i++) {
				...
				switch(val.type) {
					...
					case Type.INTEGER: result.Add(keys[i], val.n + ""); break;
                    case Type.FLOAT: result.Add(keys[i], val.f + ""); break;
